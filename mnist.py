...
if __name__ == "__main__":
    ...
    # Polynomial expansion AFTER PCA
    # If you REALLY want polynomial features, you can do:
    #    X_train_pca = PHI(X_train_pca, 2)
    #    X_test_pca  = PHI(X_test_pca, 2)
    # But be careful with memory usage. For demonstration, I'll comment out:
    X_train_pca = PHI(X_train_pca, 5)
    X_test_pca  = PHI(X_test_pca, 5)
    ...
    #     "image_index": np.arange(len(y_pred)),
    #     "true_label": y_test,
    #     "pred_label": y_pred
    # })
    # results_df.to_csv("mnist_predictions.csv", index=False)
